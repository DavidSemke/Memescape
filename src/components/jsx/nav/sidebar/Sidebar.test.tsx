...
it("Independent elements", () => {
  ...
  expect(screen.getByRole("link", { name: "Find Memes" })).toHaveAttribute(
    "href",
    "/memes",
  )
  expect(screen.getByRole("link", { name: "Create Meme" })).toHaveAttribute(
    "href",
    "/memes/create",
  )
})
...
describe("Prop dependent elements", () => {
  it("Valid session user", () => {
    ...
    expect(screen.getByRole("link", { name: "Your Memes" })).toHaveAttribute(
      "href",
      `/memes?user-id=${sessionUser.id}`,
    )

...
const fetchAction: DeepImageGridFetchAction = async (query, page, pageSize) => {
  "use server"
...
describe("Prop dependent elements", () => {
  it("addInitImages === false", async () => {
    ...
    const images = screen.queryAllByRole("img", { name: /page\d+-image\d+/ })
...
it("Load more images action", async () => {
  ...
  const images = await screen.findAllByRole("img", { name: /page\d+-image\d+/ })

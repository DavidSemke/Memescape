...
export default async function MemePage({
  ...
  if (
    mainMeme === null ||
    (mainMeme.private && sessionUserId !== mainMeme.user!.id)
  ) {
    notFound()
  }
  ...
  async function shallowGridFetch(page: number, pageSize: number) {
    ...
    const relatedMemes = await getRelatedMemes(mainMeme, page, shortCount)
  ...
  return (
    <main className="min-h-screen-ex-topbar">
      ...
      <section className="w-full">
        <h2 className="mb-4 border-b-2 border-stress-secondary pb-2">
          More Memes
        </h2>
        <ShallowImageGrid

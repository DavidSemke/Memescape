...
*/
export function nestMeme(joinedItem: JoinedMeme | JoinedBookmark): NestedMeme {
  ...
  for (const key in nestedMeme) {
    ...
    if (
      ...
      product_image.alt = memeAlt(nestedMeme.template!.name, nestedMeme.text)
...
export async function nestTemplate(
  template: Template,
...
export function processImageNoAlt(image: Image): Omit<ProcessedImage, "alt"> {
  return {
    ...
    base64: base64String(image.data, image.mime_type),
  }
}
...
export function memeDownloadName(templateName: string, mime_type: string) {
  return [templateName, mime_type.split("/")[1]].join(".").replaceAll(" ", "-")
}

export function memeAlt(templateName: string, memeText: string[]) {
  return [templateName, memeText.join(". ")].join(". ")
}

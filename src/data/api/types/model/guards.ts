import { ProcessedImage, NestedBookmark, NestedUser, NestedMeme } from "./types"
...
function isTemplate(value: any): value is Template {
  return (
    ...
    value.keywords.every((item: unknown) => typeof item === "string") &&
...
export function isNestedUser(value: any): value is NestedUser {
  return (
    ...
    (value.profile_image === undefined || isProcessedImage(value.profile_image))
  )
}
...
export function isNestedMeme(value: any): value is NestedMeme {
  return (
    ...
    (value.template === undefined || isTemplate(value.template)) &&
    (value.user === undefined || isNestedUser(value.user))
  )
}
...
export function isNestedBookmark(value: any): value is NestedBookmark {
  return (
    ...
    (value.user === undefined || isNestedUser(value.user)) &&
    (value.meme === undefined || isNestedMeme(value.meme))
  )
}

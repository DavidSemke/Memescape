...
export async function getOneMeme(id: string): Promise<NestedMeme | null> {
  ...
  try {
    const [meme = null] = await prisma.$queryRawUnsafe<JoinedMeme[]>(query, id)
...
export async function getRelatedMemes(
  ...
  const wherePredicates = [`m.id <> '${meme.id}'`]
  ...
  if (nouns.length) {
    ...
    wherePredicates.push(
      `(${predicates.join(" OR ")}` + ` OR t.id = \'${meme.template_id}\')`,
    )
  }
...
export async function postMeme(
  ...
  const parseObject: Record<string, FormDataEntryValue | null> = {
  ...
  const parse = await createMemeSchema(lineCount, true).safeParseAsync(
    parseObject,
  )
...
export async function generateMemeImage(
  ...
  return {
    ...
    alt: memeAlt(template.name, text),
    downloadName: memeDownloadName(template.name, mime_type),
  }
}

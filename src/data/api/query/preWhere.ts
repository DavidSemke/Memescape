...
function memeAliasCols() {
  return aliasCols(Object.values(Prisma.MemeScalarFieldEnum), "m")
}

function templateAliasCols() {
  return aliasCols(Object.values(Prisma.TemplateScalarFieldEnum), "t")
}

function userAliasCols() {
  return aliasCols(Object.values(Prisma.UserScalarFieldEnum), "u")
}

function imageAliasCols(prefix: "ui" | "mi") {
  return aliasCols(Object.values(Prisma.ImageScalarFieldEnum), prefix)
}

function bookmarkAliasCols() {
  return aliasCols(Object.values(Prisma.BookmarkScalarFieldEnum), "b")
}
...
export function preWhereMemeQuery() {
  ...
  const select = colGroups.map((colGroup) => colGroup.join(", ")).join(", ")

  return `SELECT ${select} FROM "Meme" as m 
...
export function preWhereUserQuery(includeProfileImage = false) {
  ...
  return `SELECT ${select} FROM "User" as u 
        ${profileImageJoin}
        `
}
...
export function preWhereBookmarkQuery() {
  ...
  return `SELECT ${select} FROM "Bookmark" as b 
